<!-- Put some ad units here -->
<!-- Yes, this file can be the same as ads.i.php. -->

<!-- geekbasic's blkads. HIGHLY RECCOMENDED; just replace the BLK address in the URL with yours! -->
<iframe src="http://www.geekbasic.com/blkads/index.php?r=B6q6aympDZyasX5TZsxxBqmWhxtu2iNen7"
 style="display:block;margin-left:auto;margin-right:auto;height:180px;width:800px"></iframe>

<!-- My a-ads code: (adaptive size)
<div style="position:fixed;bottom:0;right:0;border-radius:1vw 0 0 0;border-top:solid;border-left:solid">
<iframe data-aa='XXXXXX' src='//acceptable.a-ads.com/XXXXXX' scrolling='no' style='border:0px; padding:0;overflow:hidden' allowtransparency='true'></iframe>
</div>
-->
